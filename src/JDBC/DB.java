	...
	public CustomerID getCustomerID(int id) {
		...
		try {
			...
			while (pResultSet.next()) {
				...
				cID.setPhone(phone);
				cID.setAccounts(getAccounts(index));
			}
			pResultSet.close();
	...
	public ArrayList<Transaction> getTransactions() {
		...
		try {
			...
			while (tResultSet.next()) {
				...
				String currencyName = getCurrencyType(curID);
				Balance newBalance = new Balance(amount, curID, currencyName);

				Transaction newTransaction = new Transaction(index, cusID, name, operaAccNum, targetAccNum, info,
						newBalance);
...
//		System.out.println("Stock size:" + stockArr.size());
	...
	public void buyShares(String secureId, String stockId, int shareNumber) {
		...
		if (exist) {
			...
			String sqlAddShare = "Update sharesinfo set ShareNumber = " + (shareHas + shareNumber) + " where SecId = '"
					+ secureId + "' and StockId = '" + stockId + "'";
			try {
			...
			String sqlInsert = "Insert into sharesinfo values ('" + newShareId + "', '" + secureId + "', '" + stockId
					+ "', " + shareNumber + ")";
			try {
				mHelper.update(sqlInsert);
...
//		select * from sharesinfo, stockinfo where sharesinfo.SecId = "2" and sharesinfo.stockId = stockInfo.ID;
		String sql = "Select * from stockinfo, sharesinfo where sharesinfo.SecID = '" + secureId
				+ "' and sharesinfo.stockId = stockInfo.ID";
		ResultSet pResultSet;
		pResultSet = mHelper.query(sql);

		try {
			while (pResultSet.next()) {
				res.add(pResultSet.getString(1) + "   " + pResultSet.getString(2) + "   " + pResultSet.getDouble(3)
	...
	public void modifyShareInfo(String secureId, String stockId, int currentAmount) {
		...
		String sqlModifyShare = "Update sharesinfo set ShareNumber = " + currentAmount + " where SecId = '" + secureId
				+ "' and StockId = '" + stockId + "'";
		try {
	...
	public void insertTransaction(Transaction t) {
		...
		String insertTransSql = "insert into transactiondetails values(null, '" + operaAccNum + "', '" + targetAccNum
				+ "', " + money + ", " + curID + ", '" + name + "', " + cusID + ", '" + info + "')";
		try {
